...
const SongDisplay = (props: SongProps) => {
	...
	const songJSX = songs.map((e: SpotifyTracksData) => {
		const playlistItems = library.map((item) => {
			return (<li key={e.id+item.id} onClick={() => addToPlaylist(item.id, e)}><span className="dropdown-item">{item.name}</span></li>)
		})
			...
			<div className="d-flex pt-2" key={e.id}>
				...
				<div className="flex-grow-1 ms-3 border-bottom">
					<p className="pt-2 mb-0 small lh-sm">
						<strong className="d-block text-gray-dark">{e.name}</strong>
						<span>{e.artists[0].name}</span>
					</p>
				</div>
				<div className='d-flex align-items-center border-bottom'>
				<div className="dropdown">
					<button className="btn btn-secondary dropdown-toggle" type="button" id="playlistAdd" data-bs-toggle="dropdown" aria-expanded="false">
						Add to playlist
					</button>
